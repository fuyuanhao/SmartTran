// 存放所有和数据库交互的函数，应该可以分离到index.js和user.js中，暂不改动
// 与.env文件中获取信息相关
    ...
    db.any("SELECT * FROM userinfo WHERE username = $1", req.body.username)
        .then((result)=> {
            if(result[0] === undefined){
                res.render('login.ejs',{title:'Log In', promptinfo: '没有该用户'});
            }else{
                ...
                }else
                {
                    ...
                    res.render('login.ejs',{title:'Log In', promptinfo: '密码不正确，请重新输入'});
                }
            }
        })
        .catch((err)=>{
                    ...
                    db.none('INSERT INTO userinfo(username, password, email, telephone) VALUES($1,$2,$3,$4)', [req.body.username, req.body.password2, req.body.email, req.body.telephone])
                        .then(()=>{
                            db.any("SELECT * FROM userinfo WHERE username = $1", req.body.username)
                                .then((result)=> {
                                    if(result[0])
                                        res.render('reg.ejs',{title:'Sign Up', promptinfo: '注册成功，请登录'});
                                    else
                                        res.render('reg.ejs',{title:'Sign Up', promptinfo: '注册失败，请重新注册'});
                                })
                        })
                }else
                    res.render('reg.ejs',{title:'Sign Up', promptinfo: '两次输入密码不一致，请重新设置'});
            } else
                res.render('reg.ejs',{title:'Sign Up', promptinfo: '该用户名已经存在'});
        })
        .catch((error)=> {
            console.log(error);
        });
